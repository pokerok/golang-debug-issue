### Repository to reproduce golang compilation hang when using [https://github.com/zonedb/zonedb](https://github.com/zonedb/zonedb) package.
...
1. Try to build the repo:

    ```bash
    $ go build \
        -v \
        -o /tmp/___go_build_golang_debug_issue \
        -gcflags "all=-N -l" \
        $GOPATH/src/github.com/pokeroklife/golang-debug-issue/main.go
     ```

1. The build hangs on compilation stage. Now in other terminal send a SIGQUIT signal
to the build process in order to generate a stack trace:

    ```bash
    pkill --signal SIGQUIT --full 'go build'
    ```

1. You should get a stack trace. Here we see that `runtime_Semacquire` function hangs for some reason.
